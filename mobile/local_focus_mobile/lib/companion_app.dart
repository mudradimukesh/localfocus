...
class _MobileShellState extends State<MobileShell> {
  ...
  String _endpoint = '';
  String _status =
      'Scan the Local Focus QR code or paste its link, then connect.';
  ...
  Future<void> _load() async {
    ...
    setState(() {
      _loading = false;
      _status = savedServerUrl.trim().isEmpty
          ? 'Scan the Local Focus QR code or paste its link, then connect.'
          : 'Saved QR link ready. Tap Save and Connect.';
    });
  }
  ...
  Future<void> _connect({bool silent = false}) async {
    ...
    if (_serverController.text.trim().isEmpty) {
      if (!silent) {
        setState(() {
          _status = 'Scan the Local Focus QR code or paste its link first.';
        });
      }
      return;
    }
    ...
    try {
      ...
      _status = silent
          ? 'Could not connect. Scan the QR code again or check the pasted Local Focus link.'
    ...
    return ListView(
      ...
      children: [
        SectionCard(
          title: 'QR connection',
          subtitle:
              'Scan or paste the Local Focus QR link from the desktop dashboard.',
          child: Column(
            children: [
              LabeledField(
                label: 'QR link',
